...
- Read-only System page with native TrueNAS alerts, host identity and uptime, OS update availability, live performance and pool activity, plus 1-hour/24-hour/7-day/30-day CPU, memory, load, network, disk I/O, ZFS ARC, and CPU-temperature history
...
Eight optional or automatic views become available after the initial connection succeeds:
...
- Lifecycle actions share the update/check lock and verify the requested state within the configured verification timeout. Automatic recovery rechecks live state, starts stopped apps without a redundant stop, and leaves already-running or starting apps alone.
- A downtime incident stays open through starting/unknown states. Recovery is announced only after a fresh healthy inventory observation; automatic recovery is limited to one attempt per incident and a 15-minute cooldown between incidents, preserved in local history.
- A TrueNAS job that reports an image-pull quota failure is identified as `REGISTRY_RATE_LIMIT`, with registry sign-in guidance, and stops the remaining update batch. Registry authentication belongs in TrueNAS, not the Command Center API-key field; see [image-pull rate-limit troubleshooting](TrueNASCommandCenter/Docs/SETUP.md#image-pull-rate-limit-or-repeated-recovery-alerts).
...
The `latest` and `production` image tags track the most recently published `production` release. Immutable release tags such as `1.1.0`, minor-channel tags such as `1.1`, and commit tags such as `sha-<commit>` are published together. Increment `VERSION` before the next production release. If a production commit still uses a version that belongs to an older commit, the publish workflow completes successfully as a documented no-op instead of sending a failure notification; no container tags move until the version is incremented. Pin the image to an exact version when reproducibility matters, or keep `latest` with `pull_policy: always` for automatic image discovery.
