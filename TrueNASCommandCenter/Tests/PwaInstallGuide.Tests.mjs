...
const installGuide = require("../CommandCenterBlazor/wwwroot/pwa-install-guide.js");

test("the web app manifest includes credentials for authenticated reverse proxies", async () => {
    const appMarkup = await readFile(new URL("../CommandCenterBlazor/Components/App.razor", import.meta.url), "utf8");

    assert.match(appMarkup, /<link\s+rel="manifest"[^>]*crossorigin="use-credentials"[^>]*>/i);
});

test("the web app manifest exposes the TrueNAS Command Center identity", async () => {
    const manifest = JSON.parse(await readFile(new URL("../CommandCenterBlazor/wwwroot/manifest.webmanifest", import.meta.url), "utf8"));

    assert.equal(manifest.name, "TrueNAS Command Center");
    assert.equal(manifest.short_name, "Command Center");
});
...
test("getGuidance explains the HTTPS requirement before browser-specific steps", () => {
    ...
    assert.equal(guidance.title, "Open the HTTPS Command Center address");
