...
internal sealed class FakeHttpClientFactory(HttpMessageHandler handler, Uri? baseAddress = null) : IHttpClientFactory
{
...
internal sealed class FakeWebSocketTransport : IWebSocketTransport
{
    ...
    {
        incoming.Writer.TryWrite(JsonSerializer.Serialize(message));
    }
}
...
internal sealed class NoopAppHealthMonitorService : IAppHealthMonitorService
{
    public Task<AppHealthEvaluationResult> EvaluateAsync(IReadOnlyCollection<string> appIds, CancellationToken cancellationToken = default) => Task.FromResult(new AppHealthEvaluationResult(appIds.Count, 0, 0, 0));
}

internal sealed class NoopGitHubMetadataService : IGitHubMetadataService
{
    public Task RefreshStaleAsync(IReadOnlyCollection<string> appIds, CancellationToken cancellationToken = default) => Task.CompletedTask;

    public Task<GitHubRepositoryCache?> GetForAppAsync(string appId, CancellationToken cancellationToken = default) => Task.FromResult<GitHubRepositoryCache?>(null);
}
