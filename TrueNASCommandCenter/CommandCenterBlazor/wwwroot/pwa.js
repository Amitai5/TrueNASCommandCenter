(() => {
    ...
    function getInstallGuidance() {
        return window.trueNasPwaInstallGuide?.getGuidance(getInstallPlatform(), window.isSecureContext) ?? {
            ...
            title: "Install TrueNAS Command Center",
    ...
    function updateInstallControls() {
        ...
        document.querySelectorAll("[data-pwa-install]").forEach((button) => {
            ...
            button.setAttribute("title", deferredInstallPrompt ? "Install TrueNAS Command Center" : "Installation options");
        });
    }
    ...
    async function requestInstall() {
        if (isInstalled) {
            setInstallStatus("TrueNAS Command Center is already installed.");
            return;
        }
    ...
    async function registerServiceWorker() {
        ...
        try {
            ...
            console.warn("TrueNAS Command Center could not register its offline fallback.", error);
            return null;
        }
    }
    ...
    async function getPushState() {
        if (!supportsPush()) {
            return {
                ...
                subscription: null,
                error: window.isSecureContext ? "This browser does not support Web Push." : "Open the Command Center over HTTPS to enable push notifications."
            };
        }
    ...
    window.addEventListener("appinstalled", () => {
        deferredInstallPrompt = null;
        isInstalled = true;
        setInstallStatus("TrueNAS Command Center was installed.");
        closeInstallOptions();
        updateInstallControls();
    });
