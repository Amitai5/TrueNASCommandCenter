    ...
    TimeProvider timeProvider) : IConfigurationBackupService
{
    ...
    // This authenticated-encryption discriminator is part of the portable backup contract.
    private const string CompatibilityAdditionalData = "TrueNasAppManager:configuration-backup:v1";
    ...
    private static BackupEnvelope ParseEnvelope(string json)
    {
        ...
        {
            throw new InvalidOperationException("The selected file is not a valid TrueNAS Command Center backup.", exception);
        }
    ...
    private static BackupEncryption Encrypt(byte[] plaintext, string password)
    {
        ...
        try
        {
            ...
            aes.Encrypt(nonce, plaintext, ciphertext, tag, Encoding.UTF8.GetBytes(CompatibilityAdditionalData));
    ...
    private static byte[] Decrypt(BackupEncryption encryption, string password)
    {
        ...
        try
        {
            ...
            try
            {
                Rfc2898DeriveBytes.Pbkdf2(passwordBytes, salt, key, PasswordIterations, HashAlgorithmName.SHA256);
                using var aes = new AesGcm(key, tag.Length);
                aes.Decrypt(nonce, ciphertext, tag, plaintext, Encoding.UTF8.GetBytes(CompatibilityAdditionalData));
                return plaintext;
            }
            finally
            {
    ...
    private static string CreateFileName(DateTimeOffset now) => $"truenas-command-center-full-recovery-{now.UtcDateTime:yyyyMMddTHHmmssZ}.json";
