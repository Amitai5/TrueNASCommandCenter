    ...
    ILogger<UpdateCoordinator> logger) : IUpdateCoordinator
{
    ...
    private static (string Code, string Message) SanitizeFailure(Exception exception)
    {
        if (exception is DbUpdateException databaseFailure)
        {
            ...
            {
                5 or 6 => ("DATABASE_BUSY", "The local database stayed busy too long. Try again; if this repeats, restart TrueNAS Command Center."),
                8 => ("DATABASE_READ_ONLY", "The /data storage volume is not writable. Verify its permissions, then restart TrueNAS Command Center."),
