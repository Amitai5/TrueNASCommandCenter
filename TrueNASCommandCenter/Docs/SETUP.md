...
The username is case-sensitive. You will enter this exact username in TrueNAS Command Center.

### Grant the required app roles

1. Open **Credentials → Groups**.
2. Click **Privileges**, then **Add**.
3. Name the privilege something descriptive, such as `TrueNAS Command Center`.
...
The Operations Inbox uses `ALERT_LIST_READ` for native alerts and `POOL_READ` for scrub/resilver activity. It can always show local app-update failures, notification failures, and imported Uptime Kuma outages. Scoped accounts see TrueNAS jobs created by their current API session. Only use a Full Admin account if the inbox must include jobs owned by every other TrueNAS session; this is optional and grants much broader access than the focused roles above.
...
Open TrueNAS Command Center and complete **Step 1: TrueNAS connection**.
...
Browser push requires an HTTPS Command Center address; only `localhost` and `127.0.0.1` receive the browser's HTTP development exception. Plain `http://truenas.local` and private-IP pages cannot request notification permission. The Command Center container also needs outbound HTTPS access to the push-service host contained in each browser subscription. On iPhone and iPad, add the PWA to the Home Screen first and enable notifications from the installed app. Push notifications intentionally contain no app name, server address, or error text while traversing a browser-vendor push service. They open the local Operations Inbox for details. Push covers attention events and new warning-or-higher inbox incidents; app-down alerts also require that app's downtime action to be **Notify Only** or **Restart and Notify**. Notification-delivery failures are recorded in the inbox without generating another push.
...
Select **Docker Hub custom apps** on the same page to browse Hot apps or search public container images. Results are always restricted to Linux images published as Docker Official Images or by Verified Publishers. Category, architecture, most-pulled, and recently-updated filters remain available. Repository details include recent tags and platforms; select a tag and copy the resulting `docker.io/<namespace>/<repository>:<tag>` reference into **TrueNAS → Apps → Discover Apps → Custom App**.
...
2. Edit the custom privilege assigned to the Command Center service account's group.
3. Add `POOL_READ` beside the existing `APPS_READ` and `APPS_WRITE` roles, then save.
4. Return to TrueNAS Command Center and select **Refresh pools** on the Dashboard. If the API session was already open when the role changed, run **Settings → TrueNAS connection → Test connection** once before refreshing.
...
Favorites and groups are local Command Center organization settings and require no additional TrueNAS role:
...
5. Use **Settings → Backup & restore → Download backup** to preserve these values and their credentials for a future reinstall.
...
To monitor an app, open its **Settings** page and choose **Notify only** or **Restart once and notify** under **When this app is down**. Health checks include the top-level app state and reported containers. Each incident sends one downtime event; a subsequent fresh, healthy inventory check sends a separate recovery event. Starting, deploying, stopping, and unknown states do not count as recovery. Automatic recovery is attempted at most once per incident, with a 15-minute cooldown between recovery jobs across incidents. It rechecks TrueNAS before acting, starts stopped apps without stopping them again, and defers action if the app is already healthy or changing state. Stops initiated from this manager enter maintenance mode and do not alert. A completed `permissions` initialization workload is neutral and appears as **Exited normally** rather than degrading a running app.

For an app with slow database initialization or migrations, let the current TrueNAS job finish before requesting another action. Lifecycle actions share the update/check lock and use **Verification timeout** to bound job and health checks. A timeout means completion could not be confirmed, not that TrueNAS cancelled the job. Check the job and affected container logs before retrying. If an app repeatedly crashes, use **Notify only** while investigating the underlying failure; changing the alert policy does not repair an application or its database.
...
Open **Settings → Backup & restore**, enter and confirm a password of at least 12 characters, and select **Download backup**. The encrypted JSON contains every saved global setting and per-app configuration, including credentials, browser push devices, Uptime Kuma mappings, policies, favorites, groups, and Web UI URLs.

To restore, install and open a fresh TrueNAS Command Center instance, select the JSON file, enter its password, select **Preview**, and then select **Restore backup**. Restore is a transactional merge by app ID: unlisted apps and existing history remain unchanged. Settings for apps not yet discovered are retained until the next inventory refresh.
...
This integration is optional and read-only. Uptime Kuma remains responsible for probes, checks, alerts, and incident history; TrueNAS Command Center imports its current report instead of recreating those features.

1. In Uptime Kuma, open **Settings → Security → API Keys** and create a Prometheus API key.
2. In TrueNAS Command Center, open **Settings → Uptime Kuma**.
3. Enter the **Connection URL** reachable from the Command Center container, for example `http://truenas.local:3001`.
...
`pull_policy: always` tells Compose to check GHCR whenever TrueNAS applies or recreates the app. It does not periodically restart a running container. If TrueNAS still has not detected the new digest, use **Apps → Configuration → Manage Container Images → Pull Image**, pull `ghcr.io/amitai5/truenascommandcenter:latest`, and then save/redeploy the custom app.

Confirm the deployed release number in the Command Center sidebar or Settings header. The same value is available at `/version`, in the container startup logs, and in the image's `org.opencontainers.image.version` label. TrueNAS may continue to label a YAML custom app as `custom`; that native label is not the application release number.
...
### Image-pull rate limit or repeated recovery alerts

If a TrueNAS job reports **you have reached your pull rate limit**, an image download was refused by the registry. The app may remain stopped after an upgrade even when its version metadata has changed. This is not a missing `APPS_WRITE` permission, and restarting repeatedly does not replenish the quota. Command Center reports this as `REGISTRY_RATE_LIMIT` and stops later updates in the same batch; it cannot remove the registry's limit.

1. In **Inbox**, search the affected app and select **All states** to include resolved TrueNAS job diagnostics. Inspect the failed original `app.upgrade` or `app.start` job, not just an older `STATE_VERIFICATION_FAILED` message. The TrueNAS host's `/var/log/app_lifecycle.log` identifies the failed image when the job summary is incomplete.
2. For Docker Hub pull limits on TrueNAS 25.10, open **Apps → Configuration → Sign-in to a Docker registry → Add Registry** and select **Docker Hub**. Enter your Docker Hub username and a dedicated read-only Docker Hub personal access token in the password field. Keep that credential in TrueNAS; do not put it in the Command Center's TrueNAS API-key field or send it in logs/chat. See the [TrueNAS registry settings](https://www.truenas.com/docs/scale/25.10/scaleuireference/apps/#docker-registries) and [Docker access-token guide](https://docs.docker.com/security/access-tokens/personal-access-tokens/).
3. Authenticated Docker Personal accounts still have a pull limit. Check usage and allow the quota window to recover; authentication is not a guarantee against future throttling. See [Docker's current limits](https://docs.docker.com/docker-hub/usage/pulls/). For a different registry, configure that registry's authentication and follow its quota policy instead.
4. While investigating, set only the affected app's update policy and **When this app is down** setting to **Notify only**. Updates then require a manual action, and downtime checks do not restart it. This prevents unattended changes while preserving outage alerts. Once downloads work and the app is healthy, restore automatic policies if desired. Avoid interrupting a running database migration, deleting app data, or repeatedly stopping/starting the app.
5. Verify the Command Center's running version in its sidebar or `/version`, then update/redeploy its container when necessary without replacing `/data`. Publishing a new image does not update an already-running container, and the manager deliberately never updates itself. Releases before 3.2.3 can misread job acknowledgements as completed operations.
